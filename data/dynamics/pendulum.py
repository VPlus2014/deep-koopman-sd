...
class Pendulum(DynamicSystemBase):
    ...
    def __init__(
        ...
        X_low = np.array([-1, -1, -3600 * math.pi], float)
        X_high = np.array([1, 1, 3600 * math.pi], float)
        ...
        self._X_space = spaces.Box(X_low, X_high, seed=seed)
        self._U_space = spaces.Box(-U_high, U_high, seed=seed)

        theta0_high = math.radians(30)
        omega0_high = 1.0 * math.pi
        X0_low = np.array([-1, -math.sin(theta0_high), -omega0_high], float)
        X0_high = np.array(
            [-math.cos(theta0_high), math.sin(theta0_high), omega0_high], float
        )
        self._X0_space = spaces.Box(X0_low, X0_high, seed=seed)
