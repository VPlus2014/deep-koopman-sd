...
class DEINA_SD(nn.Module):
    ...
    &= \sum_i [g_i(x)+\psi_i(x)(h(u)-h(0))] v^i\\
    ...
    def __init__(self, params: Dict):
        ...
        enc_x_shape: List[int] = params["enc_shape"]
        ...
        self.use_enc_u = params.get("use_enc_u", False)
        self.use_complex = params.get("use_complex", False)
        self.use_affine = params.get("use_affine", False)
        ...
        # g(x,u)=g(x) + (W+h(x))*[\psi(u)-\psi(0)]
        if self.use_affine:
            assert (
                aux_shape[-1] == self.dimH * dimUh
            ), f"expect aux_shape[-1]==dimH({self.dimH})*dimUh({dimUh}), but got {aux_shape[-1]}"
            aux_params = deepcopy(params)
            aux_params["use_rbf"] = params.get("enc_rbf", False)
...
class DEINA_JBF(nn.Module):
    ...
    def __init__(self, params: Dict):
        ...
        enc_shape: List[int] = deepcopy(params["enc_shape"])
        self.n_aux = int(
            enc_shape[-1] // 2
        )  # 复值特征函数个数 M, 依照线性无关假设, 它们在任意正交基下的坐标维度应不低于 M
        self.dim = enc_shape[0]
        self.ldim = enc_shape[-1]
        assert self.ldim % 2 == 0, "X Encoder output shape must be even"

        enc_u_shape: List[int] = params.get("enc_u_shape")
        self.dimU = enc_u_shape[0]
        assert (
        ...
        enc_params["use_rbf"] = self.aux_rbf
        for k in range(self.n_aux):
            self.x_aux_nets.append(get_encoder(enc_params, enc_shape, "auc_"))
