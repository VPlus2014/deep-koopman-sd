...
def kooc_eval(model, init_conds, x_goal, T, Qs, r=1):
    ...
    x_cost_m, u_cost_m = [], []
    for Q in tqdm(Qs):
        x_kooc, u_kooc, t, t_kooc = kooc.simulate(
            init_conds, x_goal, T, Q, r, show=False
        )
...
def ilqr_eval(init_conds, x_goal, T, Qs, r=1):
    ...
    x_cost_m, u_cost_m = [], []
    for Q in tqdm(Qs):
        x_ilqr, u_ilqr, t = ilqr.simulate(init_conds, x_goal, T, Q, r, show=False)
        ...
        for j in range(4):
            ...
            for k in range(t):
                for m, arg in enumerate(args):
                    (ln,) = ax[i, j].plot(
                        [0, np.sin(arg[idx, k, 0])],
                        [0, -np.cos(arg[idx, k, 0])],
                        color=colors[m],
                        lw=4,
                    )
                    lns[k].append(ln)
            ...
            ax[i, j].set_xlim([-1, 1])
            ax[i, j].set_ylim([-1, 1])
    plt.figlegend(lines, labels, loc="upper left", labelspacing=0.0, fontsize=30)
